/* global React, I, LB, Recharts, Button, StatusBadge, StatusDot, Tag, Tabs,
   ThSort, InlineBanner, Sparkline, Hint, useToast, cx */

// Run detail — most information-dense page.
// Breadcrumb + status, horizontal step timeline (live), tabs:
// Summary / Errors / Reconciliation / Raw log. SSE-style timer ticks
// move the in-flight step forward and bump live counters.

function RunDetail({ job, onBack }) {
  const j = job || LB.JOBS[1]; // default to the "running" one
  const [tab, setTab] = React.useState('summary');
  const [now, setNow] = React.useState(0); // tick counter
  const [errCount, setErrCount] = React.useState(14);
  const [rowsLoaded, setRowsLoaded] = React.useState(184_220);
  const [stepIdx, setStepIdx] = React.useState(2); // currently "extract"
  const [stepProgress, setStepProgress] = React.useState(40);
  ...
  React.useEffect(() => {
    const t = setInterval(() => {
      ...
      setStepProgress(p => {
        if (p >= 100) {
          setStepIdx(idx => Math.min(idx + 1, LB.STEPS.length - 1));
          return 0;
        }
  ...
  const stepDurations = [3.2, 1.8, 84, 142, 8, 0.6]; // seconds, illustrative
  const totalDuration = stepDurations.slice(0, stepIdx).reduce((a, b) => a + b, 0)
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-8 pt-6 pb-4 border-b border-border dark:border-d-border bg-bg dark:bg-d-bg">
        <div className="flex items-center gap-2 text-sm text-text-muted dark:text-d-text-muted">
          ...
          <button onClick={onBack} className="hover:text-text dark:hover:text-d-text font-mono">{j.code}</button>
          <I.chevronRight size={12} />
        ...
        <div className="mt-2 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <StatusBadge status={stepIdx >= LB.STEPS.length - 1 && stepProgress >= 100 ? 'succeeded' : 'running'} />
            ...
            <span className="text-sm text-text-muted dark:text-d-text-muted flex items-center gap-1.5">
              <I.user size={12} />
              Triggered by <span className="text-text dark:text-d-text">{j.owner}</span>
            </span>
            <span className="text-xs text-text-subtle dark:text-d-text-subtle font-mono">started 2026-05-16 14:42:11 UTC</span>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="md" iconLeft={<I.terminal size={14} />}>Open in console</Button>
            <Button variant="secondary" size="md" iconLeft={<I.x size={14} />}>Cancel run</Button>
            <Button variant="primary" size="md" iconLeft={<I.refresh size={14} />}>Run again</Button>
          </div>
        </div>
...
function StepTimeline({ current, progress, durations }) {
  return (
            ...
            <div className="flex flex-col items-center gap-1.5 min-w-[110px]">
              <div className="flex items-center gap-2">
                <span className={cx(
                  ...
                  upcoming && 'bg-surface dark:bg-d-surface border-border-strong dark:border-d-border-strong text-text-subtle dark:text-d-text-subtle',
                )}>
                  {done
                    ? <I.check size={11} stroke={2.5} />
                    : active
...
function RunSummary({ j, rowsLoaded, errCount }) {
  // simple two-column key-value + sparkline
  const kv = [
  ...
  return (
    <div className="grid grid-cols-12 gap-4">
      ...
      <div className="col-span-5 grid grid-cols-2 gap-4 auto-rows-min">
        <KpiTile label="Rows loaded"     value={rowsLoaded.toLocaleString('en-US')} sub="of est. 412,048" tone="brand" />
        <KpiTile label="Throughput"      value="9,840"  sub="rows/sec (avg last 30s)" />
        <KpiTile label="Errors"          value={String(errCount)} sub="14 unique codes" tone={errCount > 12 ? 'warning' : 'default'} />
        ...
        <div className="col-span-2 border border-border dark:border-d-border rounded-md bg-surface dark:bg-d-surface">
          <div className="px-4 py-2.5 border-b border-border dark:border-d-border flex items-center justify-between">
            ...
            <span className="text-xs text-text-subtle dark:text-d-text-subtle font-mono tabular">last 60s</span>
          </div>
          <div className="px-4 py-3">
            <Sparkline data={LB.RPS_SERIES} color="#2563EB" height={64} />
...
function KpiTile({ label, value, sub, tone = 'default' }) {
  const accent = tone === 'brand' ? 'text-brand' : tone === 'warning' ? 'text-warning' : tone === 'danger' ? 'text-danger' : 'text-text dark:text-d-text';
  return (
    <div className="border border-border dark:border-d-border rounded-md bg-surface dark:bg-d-surface p-3">
      <div className="text-xs text-text-muted dark:text-d-text-muted uppercase tracking-wide">{label}</div>
...
function RunErrorsTable({ errCount }) {
  const [selected, setSelected] = React.useState(LB.ERRORS[0]);
  const rows = LB.ERRORS.slice(0, Math.max(12, Math.min(errCount, LB.ERRORS.length)));
  ...
  return (
    <div className="grid grid-cols-12 gap-4 h-full">
      <div className="col-span-7 border border-border dark:border-d-border rounded-md bg-surface dark:bg-d-surface overflow-hidden flex flex-col">
        <div className="px-3 py-2 border-b border-border dark:border-d-border flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button variant="ghost" size="sm" iconLeft={<I.copy size={12} />}>Copy</Button>
          </div>
        </div>
            ...
            <thead>
              <tr>
                {['Sev','Code','Message','Source PK','Captured'].map(h =>
                  <th key={h} className="h-8 px-3 text-left text-xs font-medium text-text-muted dark:text-d-text-muted uppercase tracking-wide border-b border-border dark:border-d-border bg-surface dark:bg-d-surface sticky top-0">{h}</th>
                )}
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <tr key={e.id} onClick={() => setSelected(e)}
                    className={cx(
                      ...
                      active ? 'bg-brand/5 dark:bg-brand/10' : 'hover:bg-surface-2 dark:hover:bg-d-surface-2',
                    )}
                    style={{ height: 32 }}>
                    <td className="px-3">
                    ...
                    <td className="px-3 truncate text-text-muted dark:text-d-text-muted">{e.message}</td>
                    <td className="px-3 font-mono text-xs text-text-muted dark:text-d-text-muted">{e.source_pk}</td>
                    <td className="px-3 text-xs text-text-muted dark:text-d-text-muted">{e.captured_at}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {/* Side panel: full JSON for selected error */}
      <div className="col-span-5 border border-border dark:border-d-border rounded-md bg-surface dark:bg-d-surface overflow-hidden flex flex-col">
        <div className="px-3 py-2 border-b border-border dark:border-d-border flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" iconLeft={<I.copy size={12} />}>Copy JSON</Button>
        </div>
  ...
  return (
    <div className="border border-border dark:border-d-border rounded-md bg-surface dark:bg-d-surface">
      <div className="px-4 py-2.5 border-b border-border dark:border-d-border flex items-center justify-between">
        ...
        <span className="text-xs text-text-subtle dark:text-d-text-subtle font-mono tabular">computed 14:47:22 UTC</span>
      </div>
      <div className="grid grid-cols-3 divide-x divide-border dark:divide-d-border">
        <ReconCol label="Source rowcount"  value="184,220"  sub="IFSAPP.CUSTOMER_INFO @ 14:42:08Z" status="ok" />
        <ReconCol label="Target rowcount"  value="184,206"  sub="stg_ifs_customer.customer_master" status="warn" />
        <ReconCol label="Variance"         value="14 rows"  sub="0.0076% — below 0.05% threshold" status="warn" />
      </div>
...
function ReconCol({ label, value, sub, status }) {
  return (
    <div className="px-4 py-3">
      <div className="text-xs uppercase tracking-wide text-text-muted dark:text-d-text-muted">{label}</div>
...
function RunRawLog({ now }) {
  const lines = React.useMemo(() => generateLog(now), [now]);
    ...
    <div className="border border-border dark:border-d-border rounded-md bg-surface dark:bg-d-surface overflow-hidden flex flex-col h-full">
      <div className="px-3 py-2 border-b border-border dark:border-d-border flex items-center justify-between">
        <div className="text-sm">
          ...
          <span className="text-text-muted dark:text-d-text-muted"> · level=info · stream=stdout</span>
        </div>
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm" iconLeft={<I.download size={12} />}>Download</Button>
          <Button variant="ghost" size="sm" iconLeft={<I.pause size={12} />}>Pause tail</Button>
        </div>
      </div>
        ...
        <pre className="px-3 py-2 font-mono text-xs leading-relaxed text-text dark:text-d-text">
          {lines.map((l, i) => (
            <div key={i} className="flex gap-3">
              <span className="text-text-subtle dark:text-d-text-subtle shrink-0 tabular">{l.ts}</span>
              <span className={cx('shrink-0 w-12 uppercase', l.level === 'error' ? 'text-danger' : l.level === 'warn' ? 'text-warning' : 'text-text-subtle dark:text-d-text-subtle')}>{l.level}</span>
              <span>{l.msg}</span>
            </div>
          ))}
        </pre>
...
function generateLog(now) {
  const base = [
    ['info','lakebridge.runner','starting run run_8842 for EXT.CUST.MASTER.DELTA'],
    ['info','lakebridge.connect','dialing oracle://ifsreader@ifs-prd-eu.corp.local:1521/IFSPROD'],
    ['info','lakebridge.connect','session established · sid=88247 · serial=39112'],
    ['info','lakebridge.count','SELECT COUNT(*) FROM IFSAPP.CUSTOMER_INFO WHERE MODIFIED_DATE > :wm'],
    ...
    ['warn','lakebridge.cast','implicit cast VARCHAR2(4000) → NVARCHAR(MAX) on column DESCRIPTION'],
    ...
    ['error','lakebridge.cast','numeric overflow casting NUMBER(22,6) → DECIMAL(18,4) on column NET_AMOUNT pk=CO-44102861'],
    ['info','lakebridge.load','flushed batch 3/19 · rows=9988 elapsed=621ms (12 quarantined)'],
  ];
  return base.concat(Array.from({ length: now % 6 }, (_, i) => [
    'info', 'lakebridge.load', `flushed batch ${4 + i}/19 · rows=10000 elapsed=${600 + (i*21) % 80}ms`,
  ])).map((l, i) => ({
    ts: '14:42:' + String(11 + i).padStart(2, '0') + '.' + String(((i*407) % 1000)).padStart(3, '0'),
    level: l[0], msg: l[1] + ' · ' + l[2],
  }));
}
