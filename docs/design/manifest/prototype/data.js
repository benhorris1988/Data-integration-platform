...
// All data is fake and module-scoped.
...
window.LB = (function () {
  ...
  const STRATEGIES = ['full_snapshot', 'append', 'watermark_delta', 'truncate_and_load'];

  // 28 jobs so the table can scroll past the visible 24
  const JOBS_SEED = [
  ...
  const JOBS = JOBS_SEED.map((r, i) => ({
    id: 'job_' + String(i+1).padStart(3, '0'),
    code: r[0], source_object: r[1], target_table: r[2], strategy: r[3],
    source: r[4], schedule: r[5], status: r[6], last_run: r[7], rows: r[8], owner: r[9],
  ...
  const ERRORS = ERRORS_SEED.map((e, i) => ({
    id: 'err_' + (10000 + i),
    severity: e[0], code: e[1], message: e[2], source_pk: e[3], captured_at: e[4],
    run_id: 'run_' + (8000 + (i % 12)),
    job_code: JOBS[i % JOBS.length].code,
  }));

  // Run timeline: connect / count / extract / load / recon / finalize
  const STEPS = ['connect','count','extract','load','recon','finalize'];

  // dashboard 24h timeline — 60 blocks
  const TIMELINE_BLOCKS = Array.from({ length: 60 }, (_, i) => {
    ...
    const dur = 30 + Math.floor(r * 240); // 30–270 sec
    return { idx: i, status, dur_sec: dur, hour: Math.floor(i/2.5), job_code: JOBS[i % JOBS.length].code };
  });

  // sparkline series
  const SPARK_RUNS   = [42,38,55,49,61,58,72,66,71,80,77,84,92,88,95,101,98,104,112,108];
  const SPARK_SUCC   = [97,97,98,96,97,98,99,98,97,98,98,99,99,98,98,99,99,98,98,99];
  const SPARK_ROWS   = [120,140,180,160,210,200,260,240,280,310,300,340,360,340,400,420,410,460,480,470];
  const SPARK_ACTIVE = [12,13,13,14,14,14,14,15,15,15,15,15,15,16,16,16,17,17,17,18];

  // rows-per-second sparkline for run detail
  const RPS_SERIES = Array.from({length: 60}, (_, i) => {
    const v = 8200 + Math.sin(i/4)*1200 + Math.cos(i/9)*900 + (i*40);
    return Math.round(v);
  });

  // Recon matrix — jobs × last 6 recon checks
  const RECON = JOBS.slice(0, 14).map((j, i) => {
    const cells = Array.from({ length: 6 }, (_, k) => {
      const seed = ((i+1)*17 + (k+1)*7) % 100;
      let s = 'ok'; if (seed > 92) s = 'fail'; else if (seed > 80) s = 'warn';
      return {
        result: s,
        src_count: 100000 + ((i*9301 + k*409) % 800000),
        tgt_count: 100000 + ((i*9301 + k*409) % 800000) - (s==='fail'? 142 : s==='warn'? 14 : 0),
        checksum_match: s === 'ok',
      };
    });
    return { job: j, cells };
  ...
  ].map((r,i) => ({ ts: r[0], actor: r[1], action: r[2], target: r[3] }));

  // Schema mapping for Job detail / Schema tab
  const SCHEMA_MAP = [
  ...
  // Source objects (for source detail child table)
  ...
  return { SOURCES, STRATEGIES, JOBS, ERRORS, STEPS, TIMELINE_BLOCKS,
           SPARK_RUNS, SPARK_SUCC, SPARK_ROWS, SPARK_ACTIVE, RPS_SERIES,
           RECON, USERS, AUDIT, SCHEMA_MAP, SOURCE_OBJECTS };
})();
